...
const syncLabel = function (queIdx) {
  ...
  item.$select.html(mustache.render(tmpl.formSelectOptions.call(this, item.columnKeys), {
...
const bindAutocompleteTarget = function (queIdx) {
  ...
  const autocompleteEvent = {
    ...
    'keyUp': function (queIdx, e) {
      ...
      if (e.which == info.eventKeys.ESC && this.activeautocompleteQueueIndex === -1) {
        ...
        this.close();
        return this;
      } else if (this.activeautocompleteQueueIndex != queIdx && e.which != info.eventKeys.BACKSPACE) {
        // 닫힌 상태 인경우
...
const clearSelected = function (queIdx) {
  ...
  this.queue[queIdx].$select.html(mustache.render(tmpl.formSelectOptions.call(this, this.queue[queIdx].columnKeys), {
...
class AX6UIAutocomplete extends AX6UICore {
  ...
  close(item) {
    ...
    jQuery(window).off("resize.ax6ui-autocomplete-" + this.instanceId).off("click.ax6ui-autocomplete-" + this.instanceId).off("keyup.ax6ui-autocomplete-" + this.instanceId);
