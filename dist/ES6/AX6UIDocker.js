...
const panelTabDragEvent = {
  "on": dragPanel => {
    if (_this.panels[0] && _this.panels[0].panels && _this.panels[0].panels.length) {
      ...
      _this.$target.on("dragover.ax5docker-" + _this.instanceId, '[data-ax6ui-docker-id="' + _this.instanceId + '"][data-ax6ui-docker-path]', function (e) {
        // todo : dragover 구현
  ...
  "off": isDrop => {
    if (isDrop) {
      let dragPanel = getPanel(_this.xvar.dragger.dragPanel.getAttribute("data-ax6ui-docker-path")),
          appendType = [];
      ...
      _this.appendPanel(dragPanel, _this.xvar.dragger.target.attr("data-ax6ui-docker-path"), appendType);
...
class AX6UIDocker extends AX6UICore {
  ...
  initOnce() {
    ...
    jQuery(window).off("resize.ax6ui-docker-" + this.instanceId).on("resize.ax6ui-docker-" + this.instanceId, U.throttle(function (e) {}, 30).bind(this));
  }
