define(["module", "AX6Util"], function (module, _AX6Util) {
    "use strict";

    var U = _interopRequireWildcard(_AX6Util);
    ...
    function eUnBind(elem, type, eventHandle) {
        ...
        if (elem.removeEventListener) {
            if (eventHandle) elem.removeEventListener(type, eventHandle);else {
                elem.removeEventListener(type);
            }
        } else if (elem.detachEvent) {
            if (eventHandle) elem.detachEvent("on" + type, eventHandle);else elem.detachEvent("on" + type);
        }
    }
    ...
    function validateEl(elem, fnName) {
        ...
        if (type === "window") return elem;else if (type === "array" && U.isElement(elem[0])) return elem;else if (type === "element") return [elem];else if (elem && elem.nodeType === 9) return [elem.documentElement];else if (elem && elem.toString() == "[object AX6Dom]") return elem.elements;else if (type !== "array" && type !== "nodelist") {
    ...
    const curCSS = function () {
        if (window.getComputedStyle) {
            return function (elem, name, num) {
                ...
                computed = window.getComputedStyle(elem, null), ret = computed ? computed.getPropertyValue(name) || computed[name] : undefined, style = elem.style;
            ...
            return function (elem, name, num) {
                ...
                computed = elem.currentStyle, ret = computed ? computed[name] : undefined, style = elem.style;
            ...
            if (el) {
                ...
                if (tagName == "html") {
                    ...
                    if (typeof d == "undefined") {
                        ...
                        let box_cond = fnName == "width" ? style(el, ["box-sizing", "padding-left", "padding-right", "border-left-width", "border-right-width"], fnName) : style(el, ["box-sizing", "padding-top", "padding-bottom", "border-top-width", "border-bottom-width"], fnName);
    ...
    function createFragment(elems) {
        let safe = safe_fragment.appendChild(doc.createElement("div")),
            tmp,
            nodes = [],
            tag,
            wrap,
    ...
    function clearElementData(el) {
        let e_hds, ei, c_el, ci, cl;

        for (let hd in el.e_hd) {
            if (typeof el.e_hd[hd] === "function") {
                eUnBind(el, hd, el.e_hd[hd]);
            } else {
                for (let ehi = 0; ehi < el.e_hd[hd].length; ehi++) eUnBind(el, hd, el.e_hd[hd][ehi]);
            }
        }
        ...
        */
        ...
        if (el.hasChildNodes()) {
            ...
            for (; ci < cl; ci++) clearElementData(c_el[ci]);
        }
    }
    ...
    function get(query, sub_query) {
        ...
        } else if (U.isWindow(query)) r_els.push(query);else if (U.isElement(query)) r_els.push(query);else if (U.isArray(query) || U.is_nodelist(query)) {
            for (i = 0; i < l; i++) if (U.isElement(query[i])) r_els.push(query[i]);
        } else if (U.isString(query) && query.substr(0, 1) === "#") r_els.push(doc.getElementById(query.substr(1)));else {
    ...
    function className(els, command, O) {
        ...
        if (command === "add" || command === "remove" || command === "toggle") {
            ...
            for (let di = 0; di < els.length; di++) {
                ...
                if (cns !== "") cns = cns.split(re_className_split);else cns = [];
                if (command === "add") {
    ...
    let dom = function dom(query) {
        let axdom = function axdom(query) {
...
//# sourceMappingURL=AX6Dom.js.map
